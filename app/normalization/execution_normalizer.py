...
def normalize_execution( message, instrument_mapper ):
    ...
    # Parse timestamp into a consistent Python datetime object.
    timestamp = None

    try:
        timestamp = parse_timestamp(message)
    except (ValueError, TypeError):
        # Missing or invalid timestamps can be handled later
        # as reconciliation exceptions instead of crashing normalization.
        timestamp = None
    ...
    return CanonicalTrade(
        ...
        source=message.get("source_system", "OMS"),
        raw_data=message
    )
